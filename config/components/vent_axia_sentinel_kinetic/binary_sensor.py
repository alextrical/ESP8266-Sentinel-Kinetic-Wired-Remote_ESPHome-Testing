...
async def to_code(config):
    ...
    if has_moving_target_config := config.get(CONF_HAS_MOVING_TARGET):
        ...
        cg.add(vent_axia_sentinel_kinetic_component.set_moving_target_binary_sensor(sens))
    if has_still_target_config := config.get(CONF_HAS_STILL_TARGET):
        ...
        cg.add(vent_axia_sentinel_kinetic_component.set_still_target_binary_sensor(sens))
    if out_pin_presence_status_config := config.get(CONF_OUT_PIN_PRESENCE_STATUS):
        ...
        cg.add(vent_axia_sentinel_kinetic_component.set_out_pin_presence_status_binary_sensor(sens))
